...
*I didn’t bother finding the old mapping file I just looked up the QZV file in the folder and used it
qiime tools view oral-taxa-bar-plots.qzv

*going back to Dr. Demmer’s suggestion of adding a pseudocount to the table before soungbird to deal with low prevalence (unfortunately the two datasets below were only for the post data)
qiime tools view pseudo_qurro.qzv
qiime tools view pseudo_qurro2.qzv
...
qiime composition add-pseudocount \
  --i-table oral-table.qza \
  --o-composition-table comp-dec2023-oral-table.qza

qiime songbird multinomial \
	--i-table comp-dec2023-oral-table.qza \
	--m-metadata-file Aspirin_Oral_Map.tsv \
	--p-formula "Contents" \
	--p-differential-prior 0.5 \
	--p-summary-interval 1 \
	--o-differentials dec2023-pseudo-differentials.qza \
	--o-regression-stats regression-stats.qza \
	--o-regression-biplot regression-biplot.qza

**This did not work, go back to “/Volumes/RAVPOWER2/ROOK_03_Professional/HHRI/Bioinf_2_Resources_C_01_Programs_01_MISSION _16S_Programs_MAR2023/01_16S_Part 45_Oral Microbiome QIIME2 QURRO pseudocount test MAR2020.docx” for instructions to add a pseudocount
...
qiime tools export \
  --input-path comp-dec2023-oral-table.qza \
  --output-path exported-pseudo-dec2023

**Need to change the name of the biom file manually in the file explorer (I force the name to be the same as the initial pseudo.qza file so that it overwrites the FeatureTable[Composition] as a FeatureTable[Frequency] artifact. (I will not filter as I want to go after treponema and corynebacterium)

qiime tools import \
  --input-path comp-dec2023-oral-table.biom \
  --output-path comp-dec2023-oral-table2.qza \
  --type 'FeatureTable[Frequency]'


Back to songbird
qiime songbird multinomial \
	--i-table comp-dec2023-oral-table2.qza \
	--m-metadata-file Aspirin_Oral_Map.tsv \
	--p-formula "Contents" \
	--p-differential-prior 0.5 \
	--p-summary-interval 1 \
...
DEC2023 ASMIC ORAL QURRO on full dataset

qiime qurro differential-plot \
  --i-ranks dec2023-pseudo-differentials.qza \
  --i-table oral-table.qza \
  --m-sample-metadata-file Aspirin_Oral_Map.tsv \
  --m-feature-metadata-file taxonomy.qza \
  --o-visualization dec2023-qurro.qzv




*Changing the table to the pseudocount table
...
qiime qurro differential-plot \
  ...
  --m-sample-metadata-file Aspirin_Oral_Map.tsv \
  --m-feature-metadata-file taxonomy.qza \
  --o-visualization dec2023-qurro-2.qzv
qiime tools view dec2023-qurro.qzv
qiime tools view dec2023-qurro-2.qzv
...
https://pubmed.ncbi.nlm.nih.gov/35444197/

After generating the Corynebacterium to treponema ratio, I decided to also generate ratios for the health associated taxa from the first analysis (Neisseria, Streptococcus, Actinomyces, Rothia) vs the disease associated taxa (Prevotella, Veillonella, Fusobacterium, Porphyromonas) since Dr. Demmer recommended we not use Ruminococcaceae as a denominator for the balances. The balances were generated against the content variable in songbird, and then boxplots for the balances were created based on the content variable. I did not want to use the collection variable since we have longitudinal data and I wanted to account for that in the LME model

The balance files were then copied to the “/Volumes/RAVPOWER2/ROOK_04_PC/MAC_Documents/qiime2/asmic_oral_test/Aspirin_Oral_Map.xlsx” file, in order to be analyzed in R via LME
