...
proc format;
value pump_perfusn 1="Yes" 2="No" 3="Unknown";
/*value transfusion 1="Yes" 2="No" 3="Unknown";
value abx_preop 1="Yes" 2="No" 3="Unknown";
value abx_preop_med___1  1="Aminoglycosides" 2="Cephalosporin" 3="Dapsone" 4="Fluoroquinolone" 5="Glycopeptides" 6="Macrolides" 7="Penicillin" 8="Sulfonamide" 9="Tetracylcine" 10="Metronidazole" 11="Other" 12="Unknown";*/
run;

data b5;
set b5;
format pump_perfusn pump_perfusn.;
/*format transfusion transfusion.;
format abx_preop abx_preop.;
format abx_preop_med___1 abx_preop_med___1.;*/
run;
...
*Running a report for B5 using the macro now;
...
*Running a report for B6 using the macro now;
...
*Running a report for B8 using the macro now;
...
data mylib.b;
merge mylib.b mylib.b2aclean mylib.b8clean mylib.b9wide;
by record_id;
run;

proc print data=mylib.b;
run;
...
PROC FREQ data =mylib.b;  TABLES record_id / noprint out=keylist; RUN; PROC PRINT;  WHERE count ge 2; RUN; 

PROC SORT data=mylib.b NODUPKEY;  BY record_id; RUN; 
...
ods excel file="/home/u63327094/analysis_MISSION_CS_data_reports/HLA_nummismatch_cs.csv";
...
*Running a report for C1 using the macro now;
...
set c2;
label 
serum_creatinine_colle = "Were 3 serum creatinine levels collected prior to or on Day 90 post-transplant?"
date_creatinine1_m3 = "1.a. Collection between day 30 and 90 - (1st collection) Date of Scr"
creatinine1_m3 = "1.b. Collection between day 30 and 90 - (1st collection) Value"
date_creatinine2_m5 = "2.a. Collection between day 30 and 90 - (2nd collection) Date of Scr"
creatinine_m6 = "2.b. Collection between day 30 and 90- (2nd collection) SCr level (mg/dL)"
date_creatinine_m4 = "3.a. Collection between day 30 and 90 - (3rd collection) Date of Scr"
creatinine_m5 = "3.b. Collection between day 30 and 90 - (3rd collection) SCr level (mg/dL)"
serum_creatinine_colle_pk = "Was serum creatinine collected prior to PK?"
date_creatinine_pk1 = "4.a. Anytime before PK date of Scr"
creatinine_pk1 = "4.b. Anytime before PK SCr level (mg/dL)"
date_creatinine_pk2 = "5.a. Anytime before PK date of Scr"
creatinine_pk2 = "5.b. Anytime before PK SCr level (mg/dL)"
;
run;
...
*some variables such as malignancies will need to have the variable format edited instead of the value format edited; 
proc format;
value serum_creatinine_colle 1="Yes" 0="No";
value serum_creatinine_colle_pk 1="Yes" 0="No";
run;

data c2;
set c2;
format serum_creatinine_colle serum_creatinine_colle.;
format serum_creatinine_colle_pk serum_creatinine_colle_pk.;
run;
...
stool_mb_rnalater
urine_mb_collect_date
urine_mb_collect_time
urine_mb_rcvd_lab_time
label_feces_sample
label_feces_dna1
label_feces_dna2
label_feces_dna3
...
label_urine_dna
label_urine_rna
label_urine_metabolites1
label_urine_metabolites2
label_urine_metabolites3
label_urine_metabolites4
label_ton_dna
label_ton_rna
label_ton_metabolite
label_hpl_dna
label_hpl_rna
label_hpl_metabolite
label_buc_dna
label_buc_rna
label_buc_metabolite
label_gin_dna
label_gin_rna
label_gin_metabolite
label_nas_dna
label_nas_rna
label_nas_metabolite
person_collecting_mb
blood_ship_dt_mb
person_shipping_mb
d2_sample_collection_and_transpo);								
...
proc export 
  data= mylib.d2widestool
  dbms=xlsx 
  outfile="/home/u63327094/analysis_MISSION_CS_data_reports/mission_demo_microbiome_cs_toshare_03012024.xlsx" 
  replace;
run;
...
*Running a report for e1 using the macro now;
...
mpa_dose_adj_diarrhea =	"Check if any of the following occurred following the date of diarrhea documented in Question 2. (Check all that apply)	"
...
format mpa_dose_adj_diarrhea	mpa_dose_adj_diarrhea.;
...
*Outputting all the resultsvariables as rtf code;

options orientation=portrait center number pageno=1 nodate;

ods rtf file="/home/u63327094/analysis_MISSION_PS_data_reports/PS_REDCAP_E3_V2.rtf";
...
%let form = e3;
%categ(possible_diarrhea,1);

proc print data = e3;
var record_id possible_diarrhea;
where possible_diarrhea = 1;
title "Checking samples with possible diarrhea reported in the EMR";
run;

%let form = e3pos;
%categ(take_mpa_diarrhea,2);
%categ(mpa_intake_diarrhea,3);
%categ(mpa_dose_adj_diarrhea,4);
%categ(diarrhea_othercause,5);
%categ(stool_culture,6);
%categ(stool_infection,7);
%categ(diarrhea_resolve,8);
...
*g2;
%selectvars(g2,										
covid19_ld_test	covid19_ld_result	
covid19_ld_test_dt	
...
value	mpa_change_how___1f	1 = "Product changed"; 
value	mpa_change_how___2f	2 = "Dose changed";
value	mpa_change_how___3f	3 = "Frequency of doses changed"; 
value	mpa_change_how___4f	4 = "All MPA products stopped";
...
proc export 
  data=h3
  dbms=xlsx 
  outfile="/home/u63327094/analysis_MISSION_06_16S_Shotgun_TAC_analysis/HGBHCT_CS_04220204.xlsx" 
  replace;
run;
...
proc export 
  data=onerectest
  dbms=xlsx 
  outfile="/home/u63327094/analysis_MISSION_02_CS_data_reports/mission_demo_cs_noexclusions.xlsx" 
  replace;
run;

data mylib.onerectest_toshare_cs;
set onerectest;
if record_id = 3001 then delete; *excluding 4003 for missing age + being withdrawn;
if record_id = 3003 then delete; *excluding 4024 for form b2 + being withdrawn;
if record_id = 3005 then delete; *excluding 3519 for form b2 pets + being withdrawn;
...
data mylib.onerectest_toshare_cs;
set mylib.onerectest_toshare_cs;
...
proc print data = mylib.onerectest_toshare_cs;
...
proc export 
  data= mylib.onerectest_toshare_cs
  dbms=xlsx 
  outfile="/home/u63327094/analysis_MISSION_02_CS_data_reports/mission_demo_cs_toshare_04012024.xlsx" 
  replace;
run;
...
ods excel file="/home/u63327094/analysis_MISSION_CS_data_reports/ATC2023_mission_demo_toshare_datadictionary_03012024.xlsx";

proc contents data =  mylib.onerectest_toshare_cs;
run;
...
*Based on Moataz and Dr. Jacobson's feedback, PID 4042 should be African American, and PID 3015 is Native American;

data onerectest_toshare_cs;
set mylib.onerectest_toshare_cs;
run;

proc contents data = onerectest_toshare_cs;
run;

data onerectest_toshare_cs;
set onerectest_toshare_cs;
...
proc print data = onerectest_toshare_cs;
var record_id tx_dt birth_yr_ld yr_ld age_ld birth_yr_dd yr_dd age_dd;
run;

proc freq data = onerectest_toshare_cs;
table predom_race;
run;
...
data onerectest_toshare_cs;
set onerectest_toshare_cs;
if record_id = 3015 then predom_race_2 = 4;
...
proc means data = onerectest_toshare_cs mean std;
var age;
run;
