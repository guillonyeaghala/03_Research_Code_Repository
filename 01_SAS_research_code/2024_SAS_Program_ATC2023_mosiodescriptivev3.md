...
data summary_scr_dsa_infxn_virome;
merge summary_scr_dsa_infxn_virome mylib.h1clean4 mylib.g9clean4;
by record_id;
run;
...
*Testing the association between MPA EHR and MOSIO diarrhea;

*Right click on the folder location and select properties to copy the path name;

libname mylib "/home/u63327094/analysis_MISSION_04_ATC2023_mosio";

*Importing the data;

proc import 
datafile= '/home/u63327094/analysis_MISSION_04_ATC2023_mosio/MPA_EHR_MOSIO_check2.xlsx'
out=mylib.mission_mosio_ehr
dbms=xlsx
replace;
getnames=yes;
run;
...
*Load data; 
data mission_mosio_ehr; 
set mylib.mission_mosio_ehr;
run; 

proc print data = mission_mosio_ehr;
run;

*****************************************************************************************************************************;

proc anova data = mission_mosio_ehr;
class everCTCAE21;
model MPA_AUC5_12_0_12 = everCTCAE21;
means everCTCAE21;
run;

proc anova data = mission_mosio_ehr;
class diarrheactcaegrade1;
model MPA_AUC5_12_0_12 = diarrheactcaegrade1;
means diarrheactcaegrade1;
run;

data mission_mosio_ehr; 
set mission_mosio_ehr;
if diarrheactcaegrade1 = 3 then delete;
run; 
...
*Categorizing EHR into tertiles;
*https://communities.sas.com/t5/SAS-Procedures/Continues-variable-into-tertiles/td-p/447513;

proc sort data = mission_mosio_ehr;
by MPA_AUC5_12_0_12;
run;

proc rank data = mission_mosio_ehr groups = 3 out=mission_mosio_ehr;
var MPA_AUC5_12_0_12;
ranks MPA_AUC5_12_0_12_tert;
run;

proc freq data = mission_mosio_ehr;
tables MPA_AUC5_12_0_12_tert;
run;

data mission_mosio_ehr;
set mission_mosio_ehr;
if MPA_AUC5_12_0_12_tert = 0 then low_EHR = 1;
else low_EHR = 0;
run;

proc freq data =  mission_mosio_ehr; 
tables low_EHR * everCTCAE21 / chisq;
run;

*Nonparametric tests to account for the distribution;
*https://stats.oarc.ucla.edu/sas/whatstat/what-statistical-analysis-should-i-usestatistical-analyses-using-sas/;

proc freq data =  mission_mosio_ehr; 
tables low_EHR * diarrheactcaegrade1 / fisher;
run;
